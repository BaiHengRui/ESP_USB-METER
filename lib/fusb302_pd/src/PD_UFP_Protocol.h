...
#define PD_PROTOCOL_EVENT_VDM_RESPONSE  (1 << 5) // 添加VDM响应事件

// 在 PD_UFP_Protocol.h 中添加
// 添加在文件顶部 (宏定义)
#define VDO_IDH(usb_host, usb_dev, ptype, is_modal) \
    (((is_modal) & 0x1) << 26 | ((ptype) & 0x3) << 24 | ((usb_dev) & 0x1) << 23 | ((usb_host) & 0x1) << 22)

#define VDO_IDH_VID(vid)       (((vid) & 0xFFFF) << 0)
#define VDO_CERT(cert)         ((cert) & 0xFFFF)
#define VDO_PRODUCT(pid, xid)  ((((xid) & 0xFF) << 16) | ((pid) & 0xFFFF))

// 添加在结构体定义区域
typedef struct {
    uint16_t vid;       // Vendor ID
    uint16_t pid;       // Product ID
    uint8_t xid;        // eXperimental ID
    uint8_t fw_version; // Firmware version
    uint8_t hw_version; // Hardware version
    bool usb_2_0;       // USB 2.0 support
    bool usb_3_1;       // USB 3.1 support
    bool usb_4;         // USB4 support
    bool tb_3;          // Thunderbolt 3 support
    bool p40;           // Power 40W+
} PD_emarker_info_t;
//E-Marker Info End
...
typedef struct {
    ...
    //E-Marker Info
    // 添加e-mark信息存储
    PD_emarker_info_t emarker_info;
    // 添加VDM响应存储
    uint16_t tx_vdm_header;
    uint32_t tx_vdm_data[7];
    //E-Marker Info End
} PD_protocol_t;
