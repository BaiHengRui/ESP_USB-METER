...
// static bool responder_vender_def    (PD_protocol_t * p, uint16_t * header, uint32_t * obj); //New Function:Responder for VDM
// New responder_vender_def function
static bool responder_vender_def(PD_protocol_t * p, uint16_t * header, uint32_t * obj)
{
    // 修改这里
    if (p->tx_vdm_header) {
        *header = p->tx_vdm_header;
        memcpy(obj, p->tx_vdm_data, sizeof(uint32_t) * 2);
        p->tx_vdm_header = 0;
        return true;
    }
    return false;
...
static const struct PD_msg_state_t data_msg_list[] PROGMEM = {
    ...
    {.name = str_VDM,           .handler = handler_vender_def,  .responder = responder_vender_def},//VDM (Vendor Defined Message) handler
...
//New handler_vender_def function 329 Lines
// 修正 VDO 构造
static void handler_vender_def(PD_protocol_t * p, uint16_t header, uint32_t * obj, PD_protocol_event_t * events)
{
    if (obj == NULL || p == NULL) return;
    
    uint32_t vdm_header = obj[0];
    uint8_t vdm_type = (vdm_header >> 0) & 0x7;
    uint8_t vdm_cmd = (vdm_header >> 6) & 0x1F;
    
    if (vdm_type == 0 && vdm_cmd == 1) { // Discover Identity
        // 正确的 VDO 构造
        uint32_t vdos[3] = {0};
        
        // ID Header VDO
        vdos[0] = (1 << 27) |    // Modal operation
                  (1 << 25) |    // Product type: Cable
                  (1 << 24) |    // USB Host Capable
                  (1 << 23) |    // USB Device Capable
                  (p->emarker_info.vid & 0xFFFF); // VID
        
        // Cert Stat VDO
        vdos[1] = 0x0000; // 证书状态
        
        // Product VDO
        vdos[2] = (p->emarker_info.pid << 16) | 
                  (p->emarker_info.xid << 8);
        
        // 存储响应数据
        p->tx_vdm_header = generate_header(p, PD_DATA_MSG_TYPE_VENDOR_DEFINED, 3);
        memcpy(p->tx_vdm_data, vdos, sizeof(uint32_t) * 3);
        
        if (events) *events |= PD_PROTOCOL_EVENT_VDM_RESPONSE;
    }
}
...
// New responder_vender_def  90 Lines
// static bool responder_vender_def(PD_protocol_t * p, uint16_t * header, uint32_t * obj)
// {
