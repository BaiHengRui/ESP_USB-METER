...
#include "PD_UFP.h"
//不同的日志事件类型，例如消息传输（TX/RX）、设备状态、CC线状态、电源状态等
enum {
    STATUS_LOG_MSG_TX,
    STATUS_LOG_MSG_RX,
    STATUS_LOG_DEV,
...
// void PD_UFP_Log_c::Listen_status_log_event(uint8_t status, const char* message, uint32_t *obj)
// {
//     if (((status_log_write - status_log_read) & STATUS_LOG_MASK) >= STATUS_LOG_MASK) {
//         return;
//     }
...
//     status_log_t * log = &status_log[status_log_write & STATUS_LOG_MASK];
//     log->custom_message = message; // 存储自定义消息
...
// 这个函数用于读取日志行，返回读取的字符数
// int PD_UFP_Log_c::Listen_status_log_readline(char * buffer, int maxlen)
// {
//     if (status_log_write == status_log_read) {
//         return 0;
//     }
...
//     case STATUS_LOG_MSG_RX: {
//         PD_msg_info_t info;
//         if (PD_protocol_get_msg_info(log->msg_header, &info)) {
//             LOG("%sRX %s id=%d\n", t, info.name, info.id);
//             if (info.num_of_obj) {
//                 status_log_counter++;
//             }
//         } else {
//             LOG("%sRX Unknown header: 0x%04X\n", t, log->msg_header);
//         }
//         break; }
        
//     case STATUS_LOG_CC:
//         LOG("%s%s\n", t, log->obj_count ? (const char*)log->obj_count : "");
//         break;
//     case STATUS_LOG_SRC_CAP:
//         n = status_log_readline_src_cap(buffer, maxlen);
//         break;
